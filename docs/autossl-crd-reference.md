...
| `contactEmail` | `string` | Yes | - | Let's Encrypt contact email |
| `proxyEndpoint` | `string` | Yes | - | Endpoint to proxy_pass requests to |
| `verificationEndpoint` | `string` | Yes | - | Endpoint used to validate if certificate generation is allowed |
| `domainWhitelist` | `string` | No | - | Domain whitelist will bypass domain verification |
| `domainBlacklist` | `string` | No | - | Domain blacklist will never get autogenerated certificates |
| `redisHost` | `string` | Yes | - | Storage adapter redis host |
| `redisPort` | `int` | No | `6379` | Storage adapter redis port |
